...
class CurrencyConverter extends Component {
    ...
    tryConvert = (state) => {
      ...
      if (isNaN(value)) {
        ...
        const euro = this.toEur(value, state.rates[state.currencies[state.updated]]);
        const result = this.fromEur(euro, state.rates[state.currencies[this.otherSide(state.updated)]]);
        ...
        () => {
          axios.get('https://api.exchangeratesapi.io/latest')
            .then(response => {
              ...
              this.setState({rates: {...response.data.rates, 'EUR': 1}, loading: false});
            });
        },
        3000
      );
    ...
    render () {
      ...
      return (
        <div className="App">
          <h1 className={modules.Heading}>{this.state.currencies['left']}/{this.state.currencies['right']} Conversion</h1>
          {display}
        </div>
      );
    }
